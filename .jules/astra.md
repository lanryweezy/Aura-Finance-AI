...
## 2026-06-30 - Enforce Token Limits with Bounded Context
**Learning:** Sending unbounded lists or large amounts of raw data (such as the entire history of transactions, bills, or invoices) into a prompt payload leads to severe token bloat. This can quickly exhaust context windows, trigger provider rate limits, and severely degrade both cost and performance without adding proportional value to the AI analysis.
**Action:** Always constrain datasets before injecting them into a prompt. Use strategies like `.slice()` to limit context arrays to a reasonable maximum (e.g., 20 invoices, 50 transactions), and provide aggregations or rollups for data exceeding these bounds if necessary. For batch jobs like categorization, chunk the data and map remaining excess to safe fallbacks (e.g., 'Uncategorized') rather than failing the entire request.
