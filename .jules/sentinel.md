...
## 2026-10-15 - CSV Injection & Structure Breaking
**Vulnerability:** User-provided inputs that start with formula indicators (`=`, `+`, `-`, `@`, `\t`, `\r`) injected into CSV exports can lead to formula injection (CSV injection) vulnerabilities if executed by spreadsheet processors like Excel or Google Sheets. Additionally, a failure to appropriately escape double quotes causes CSV file structures to break when fields contain delimiters, newlines, or double quotes themselves.
**Learning:** `val.includes(",")` followed by wrapping in double quotes is insufficient for escaping CSV values according to RFC 4180. Double quotes must be doubled (`""`) when placed inside a quoted field, and a single quote prefix (`'`) can mitigate formula execution.
**Prevention:** Always prepend formula injection markers with a single quote (`'`) and sanitize CSV structures by escaping double quotes (`.replace(/"/g, '""')`) and enclosing any strings containing delimiters, newlines, or quotes inside double quotes before exporting native CSV files.
