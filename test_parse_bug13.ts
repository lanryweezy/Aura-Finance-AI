  ...
  try {
    ...
    try {
      ...
      if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
          objStr = text.slice(firstBrace, lastBrace + 1);
      }

      let arrStr = "";
      if (firstBracket !== -1 && lastBracket !== -1 && lastBracket > firstBracket) {
          arrStr = text.slice(firstBracket, lastBracket + 1);
      }

      // Try parsing both and return the one that succeeds and is longer
      let parsedObj: any = null;
      let parsedArr: any = null;

      try {
          if (objStr) parsedObj = JSON.parse(objStr);
      } catch (e) {}

      try {
          if (arrStr) parsedArr = JSON.parse(arrStr);
      } catch (e) {}
