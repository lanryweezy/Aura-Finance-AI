...
export const receiptOcrService = {
  scanReceipt: async (imageUrl: string, imageFile?: File): Promise<ReceiptScan> => {
    ...
    try {
      ...
      const systemInstruction = `Analyze this receipt and extract: merchant name, date (YYYY-MM-DD), total amount, VAT amount, category, description, and line items (description + amount). Return as JSON.`;

      const response = await withTimeout(aiClient.models.generateContent({
        model: 'gemini-2.0-flash',
        contents: [{ role: 'user', parts: [imagePart, { text: "Extract data from this receipt." }] }],
        config: {
          // AI Quality: Extracted persona and formatting constraints to systemInstruction
          // to improve model adherence and prevent chatty responses.
          systemInstruction,
