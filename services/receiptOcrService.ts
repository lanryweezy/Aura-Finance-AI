    ...
    try {
      ...
      const response = await withTimeout(() => aiClient.models.generateContent({
        ...
        config: {
          ...
          systemInstruction: `Analyze this receipt and extract: merchant name, date (YYYY-MM-DD), total amount, VAT amount, category, description, and line items (description + amount). Return as JSON.`,
