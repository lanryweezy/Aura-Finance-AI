  ...
  scanReceipt: async (imageUrl: string, imageFile?: File): Promise<ReceiptScan> => {
    ...
    try {
      ...
      const response = await withTimeout(aiClient.models.generateContent({
        ...
        config: {
          // AI Quality: Extracted persona and structural output constraints into systemInstruction to improve model adherence and prevent prompt injection.
          systemInstruction: `You are an AI assistant. Analyze this receipt and extract: merchant name, date (YYYY-MM-DD), total amount, VAT amount, category, description, and line items (description + amount). Return as JSON.`,
