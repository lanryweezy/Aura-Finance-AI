...
export const aiAutoCategoryService = {
  ...
  categorizeWithAI: async (transactions: CategorizedTransaction[], categories: string[]): Promise<AutoCategoryResult[]> => {
    ...
    let allResults: AutoCategoryResult[] = [];
    const CHUNK_SIZE = 50;
...
${JSON.stringify(chunk.map(t => ({ id: t.id, narration: t.narration, amount: t.amount, type: t.type })))}`;
        ...
        allResults = allResults.concat(results.map((r: any) => ({
          ...r,
          originalCategory: 'Uncategorized',
        })));
      } catch (error) {
        monitoringService.trackError('AI_AUTO_CATEGORY', error as Error);
        // AI Quality: If a chunk fails, fallback to rules for that chunk only, preventing the entire batch from failing.
        allResults = allResults.concat(aiAutoCategoryService.categorizeByRules(chunk));
      }
    }
