...
export const AICFOInsights: React.FC<AICFOInsightsProps> = ({ analysis, isLoading }) => {
    return (
        <Card className="bg-white dark:bg-dark-primary h-full border-gray-100 dark:border-white/5 shadow-xl">
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center h-full min-h-[300px] bg-gray-50 dark:bg-dark-secondary/50 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700">
                    <Spinner />
                    <p className="mt-4 text-sm font-bold text-gray-500 dark:text-gray-400 animate-pulse">O-Heidi is analyzing performance...</p>
                </div>
            ) : (
               <div className="text-gray-700 dark:text-gray-300 space-y-4 whitespace-pre-wrap text-sm leading-relaxed bg-gray-50 dark:bg-dark-secondary/30 p-5 rounded-2xl border border-gray-100 dark:border-white/5 shadow-inner">
                {analysis}
               </div>
            )}
        </Card>
