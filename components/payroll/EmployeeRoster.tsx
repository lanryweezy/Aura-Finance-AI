            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead className="bg-aura-gray-50 dark:bg-dark-tertiary">
                        <tr>
                            <th className="p-4 text-xs font-black uppercase tracking-widest text-aura-gray-500 dark:text-gray-400">Employee</th>
                            <th className="p-4 text-xs font-black uppercase tracking-widest text-aura-gray-500 dark:text-gray-400">Job Title</th>
                            <th className="p-4 text-xs font-black uppercase tracking-widest text-aura-gray-500 dark:text-gray-400 text-right">Gross Salary</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {employees.map(emp => (
                            <tr key={emp.id} className="hover:bg-aura-gray-50/50 dark:hover:bg-dark-secondary/50 transition-colors group">
                                <td className="p-4 text-aura-gray-900 dark:text-white font-bold">
                                    <div className="flex flex-col">
                                        <span>{emp.name}</span>
                                        <span className="text-xs text-aura-gray-500 dark:text-gray-500 font-medium">{emp.email}</span>
                                    </div>
                                </td>
                                <td className="p-4 text-sm text-aura-gray-600 dark:text-gray-300 font-medium">{emp.jobTitle}</td>
                                <td className="p-4 font-mono text-sm text-aura-gray-900 dark:text-white font-bold text-right">{formatAmount(emp.grossSalary)}</td>
                 ...
                 {employees.length === 0 && (
                     <div className="flex flex-col items-center justify-center p-12 text-center">
                         ...
                         <p className="text-gray-500 dark:text-gray-400 font-medium italic">No employees added yet.</p>
                     </div>
                 )}
            </div>
        </Card>
